...
const Modal = ({ isOpen, isEdit, closeModal, setUserValue, editingId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      // edit mode on!
      const edit = await user.functions.editUser(value)
      // console.log(edit)
      setUserValue(edit._id)
      setValue({ name: "", location: "", email: "" });
      closeModal();
    } else {
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      const getSingleUser = async () => {
        const getUser = await user.functions.getSingleUser( new BSON.ObjectID(editingId))
        // console.log(getUser)
        setValue(getUser)
      }
